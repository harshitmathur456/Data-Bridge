    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/import`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            rows: chunks[i], fileName: files.map(f => f.name).join(", "), isFinalBatch: isFinalBatch && failedChunks.length === 0,
    ...
    for (let i = 0; i < failedChunks.length; i++) {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/import`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            rows: item.chunk, fileName: files.map(f => f.name).join(", "), isFinalBatch,
            allImported: isFinalBatch ? imp : undefined,
            allSkipped: isFinalBatch ? skip : undefined,
            totalRows: rawRows.length, aiEngine: engine,
          }),
            ...
            <div className="flex flex-col items-center justify-center min-h-full p-10"
              ...
              <div className="w-full max-w-xl relative">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {files.map(f => (
                      <div key={f.id} className={`${cardCls} p-4 flex items-center gap-4 shadow-sm`}>
                        ...
                        <button onClick={() => removeFile(f.id)} className="p-2 rounded-lg text-muted-color hover:bg-red-100 hover:text-red-500 transition-colors">
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                    ))}
                ...
                <div className="flex justify-end mt-6">
                  <button disabled={files.length === 0 || isParsing} onClick={() => setStep("preview")}
